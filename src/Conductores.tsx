...
function Conductores({ onVolver }: Props) {
  ...
  const cargarDatos = async () => {
    ...
    while (true) {
      let query = supabase
        ...
        .select("conductor, id_vehiculo, centro_costos, proveedor, fecha, litros, recorrido, rendimiento_estandar, rendimiento_real, costo_litro, costo_total, cumplimiento, estado_transaccion");
  ...
  const resumenPorConductor: FilaConductor[] = (() => {
    ...
    for (const row of datos) {
      ...
      if (!mapa[conductor]) {
        mapa[conductor] = {
          conductor, cargas: 0, ok: 0, abajo: 0, arriba: 0, pctCumplimiento: 0,
          dineroEnRiesgo: 0, costoTotal: 0, incidencias: 0,
          sumaRecorrido: 0, sumaLitros: 0, sumaLitrosPorEstandar: 0, filas: [],
          unidades: new Set(), centrosCostos: new Set(), proveedores: new Set()
        };
      }
  ...
  const rendEstandarUnico = unSoloConductor && unSoloConductor.sumaLitros > 0 ? unSoloConductor.sumaLitrosPorEstandar / unSoloConductor.sumaLitros : 0;
  const rendRealUnico = unSoloConductor && unSoloConductor.sumaLitros > 0 ? unSoloConductor.sumaRecorrido / unSoloConductor.sumaLitros : 0;
        ...
        {!unSoloConductor ? (
          <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "24px", border: `1px solid ${TEMA.borde}`, textAlign: "center" }}>
            <p style={{ color: TEMA.textoSecundario, margin: 0 }}>
              {resumenPorConductor.length === 0
                ? "No hay conductores con estos filtros."
                : `Hay ${resumenPorConductor.length} conductores en este resultado — usa el filtro de "Conductor" para elegir uno solo y ver sus indicadores (así no se promedian cosas de conductores distintos).`}
            </p>
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(8, 1fr)", gap: "16px" }}>
            <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "20px", borderTop: `4px solid ${TEMA.naranja}` }}>
              ...
              <h2 style={{ color: TEMA.textoPrincipal, margin: "8px 0 0", fontSize: "24px" }}>{rendEstandarUnico.toFixed(2)} <span style={{ fontSize: "12px" }}>km/L</span></h2>
            </div>
            <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "20px", borderTop: `4px solid ${TEMA.naranja}` }}>
              <p style={{ color: TEMA.textoSecundario, margin: 0, fontSize: "14px" }}>Rend. Real</p>
              <h2 style={{ color: TEMA.textoPrincipal, margin: "8px 0 0", fontSize: "24px" }}>{rendRealUnico.toFixed(2)} <span style={{ fontSize: "12px" }}>km/L</span></h2>
            </div>
            <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "20px", borderTop: `4px solid ${TEMA.naranja}` }}>
              <p style={{ color: TEMA.textoSecundario, margin: 0, fontSize: "14px" }}>Cargas OK</p>
              <h2 style={{ color: TEMA.textoPrincipal, margin: "8px 0 0", fontSize: "24px" }}>{unSoloConductor.ok}</h2>
            </div>
            <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "20px", borderTop: `4px solid ${TEMA.naranja}` }}>
              <p style={{ color: TEMA.textoSecundario, margin: 0, fontSize: "14px" }}>Cargas POR ABAJO</p>
              <h2 style={{ color: TEMA.rojo, margin: "8px 0 0", fontSize: "24px" }}>{unSoloConductor.abajo}</h2>
            </div>
            <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "20px", borderTop: `4px solid ${TEMA.naranja}` }}>
              <p style={{ color: TEMA.textoSecundario, margin: 0, fontSize: "14px" }}>Cargas POR ARRIBA</p>
              <h2 style={{ color: TEMA.naranjaOscuro, margin: "8px 0 0", fontSize: "24px" }}>{unSoloConductor.arriba}</h2>
            </div>
            <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "20px", borderTop: `4px solid ${TEMA.naranja}` }}>
              <p style={{ color: TEMA.textoSecundario, margin: 0, fontSize: "14px" }}>Costo Total</p>
              <h2 style={{ color: TEMA.textoPrincipal, margin: "8px 0 0", fontSize: "20px" }}>{fmtDinero(unSoloConductor.costoTotal)}</h2>
            </div>
            <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "20px", borderTop: `4px solid ${TEMA.naranja}` }}>
              <p style={{ color: TEMA.textoSecundario, margin: 0, fontSize: "14px" }}>$ en Riesgo</p>
              <h2 style={{ color: TEMA.rojo, margin: "8px 0 0", fontSize: "20px" }}>{fmtDinero(unSoloConductor.dineroEnRiesgo)}</h2>
            </div>
            <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "20px", borderTop: `4px solid ${TEMA.naranja}` }}>
              <p style={{ color: TEMA.textoSecundario, margin: 0, fontSize: "14px" }}>Total Registros</p>
              <h2 style={{ color: TEMA.textoPrincipal, margin: "8px 0 0", fontSize: "24px" }}>{unSoloConductor.cargas}</h2>
            </div>
          </div>
        )}
      </div>
        ...
        {cargando ? <p style={{ color: TEMA.textoSecundario }}>Cargando datos...</p> : (
          <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "24px", border: `1px solid ${TEMA.borde}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h3 style={{ color: TEMA.textoPrincipal, marginTop: 0 }}>Resumen por Conductor ({resumenPorConductor.length})</h3>
              <button onClick={() => descargarCSV(resumenPorConductor.map(f => ({
              ...
              })), "conductores")} style={{ backgroundColor: TEMA.verde, border: "none", color: "white", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontSize: "13px", fontWeight: "bold" }}>
                📥 Descargar Excel
              </button>
            </div>
            <p style={{ color: TEMA.textoSecundario, fontSize: "13px", marginTop: "4px", marginBottom: "16px" }}>Da clic en un conductor para ver el detalle de cargas.</p>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "1100px" }}>
                ...
                <tbody>
                  {resumenPorConductor.map((fila, i) => {
                    ...
                    return (
                      <>
                        <tr key={i} onClick={() => setConductorExpandido(expandido ? null : fila.conductor)} style={{ borderBottom: `1px solid ${TEMA.borde}`, cursor: "pointer" }}>
                          <td style={{ padding: "12px" }}>{expandido ? "▼" : "▶"} {fila.conductor}</td>
                          ...
                          <tr>
                            <td colSpan={11} style={{ padding: "16px 24px", backgroundColor: "#ffffff" }}>
                              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "24px", marginBottom: "16px" }}>
                                <div>
                                  <p style={{ color: TEMA.textoSecundario, fontSize: "12px", margin: "0 0 8px", fontWeight: "bold" }}>Unidades ({fila.unidades.size})</p>
                                  {[...fila.unidades].map(u => <p key={u} style={{ fontSize: "13px", margin: "0 0 4px" }}>{u}</p>)}
                                </div>
                                <div>
                              ...
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                                ...
                                <button onClick={() => descargarCSV(fila.filas.slice().sort((a, b) => (a.fecha || "").localeCompare(b.fecha || "")).map(c => ({
                                  Fecha: c.fecha, Unidad: c.id_vehiculo,
                                ...
                                })), `detalle_conductor_${fila.conductor}`)} style={{ backgroundColor: TEMA.verde, border: "none", color: "white", padding: "6px 14px", borderRadius: "8px", cursor: "pointer", fontSize: "12px", fontWeight: "bold" }}>
                                  📥 Descargar detalle
                                </button>
                              </div>
                              <div style={{ overflowX: "auto" }}>
                                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "1000px" }}>
                                  <thead>
                                    <tr style={{ borderBottom: `1px solid ${TEMA.borde}` }}>
                                      ...
                                      <th style={{ color: TEMA.textoSecundario, textAlign: "left", padding: "8px", fontWeight: "normal", fontSize: "12px" }}>Unidad</th>
                                    ...
                                    {fila.filas.slice().sort((a, b) => (a.fecha || "").localeCompare(b.fecha || "")).map((c, j) => {
                                      ...
                                      return (
                                        <tr key={j} style={{ borderBottom: `1px solid ${TEMA.borde}` }}>
                                          ...
                                          <td style={{ padding: "8px", fontSize: "13px" }}>{c.id_vehiculo}</td>
