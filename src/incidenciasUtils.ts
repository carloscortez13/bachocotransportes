// Motor compartido para calcular "incidencias" por unidad.
//
// REGLA DE NEGOCIO:
// Una incidencia ocurre cuando, en un MES completo, el rendimiento ponderado
// de una unidad (suma de recorrido ÷ suma de litros de ese mes) queda FUERA
// del rango permitido del estándar (por abajo del 85% o por arriba del 115%).
// No se marca incidencia por una sola carga mala — se evalúa el mes completo.
//
// Para cada renglón (carga) se calcula:
//   - _pctCumplAcumulado: % de cumplimiento acumulado de esa unidad, tomando
//     en cuenta todas sus cargas desde la más antigua hasta esa fecha.
//   - _incidenciasAcumuladas: cuántos meses-incidencia llevaba esa unidad
//     acumulados hasta ese momento (incluyendo el mes de esa carga).
//   - _esIncidenciaDelMes: si el mes al que pertenece esa carga fue, en sí,
//     un mes de incidencia para esa unidad (útil para agregar por centro/conductor).
...
export interface CargaEnriquecida {
  ...
  export function calcularIncidencias(datos: any[]): CargaEnriquecida[] {
    ...
    for (const unidad in porUnidad) {
      ...
      // Agrupar las cargas de esta unidad por mes (YYYY-MM)
      const porMes: Record<string, any[]> = {};
      for (const row of filas) {
        const mes = (row.fecha || "").slice(0, 7); // "2026-03"
      ...
      for (const mes of mesesOrdenados) {
        ...
        // Cumplimiento acumulado carga por carga (cronológico) dentro de este mes y anteriores
        for (const row of cargasDelMes) {
          resultado.push({
            ...row,
            _pctCumplAcumulado: 0, // se calcula abajo con el acumulado real
    ...
    // 2. Calcular % de cumplimiento acumulado carga por carga (cronológico, por unidad)
  ...
  // Cuenta cuántos meses-incidencia distintos (unidad + mes) aparecen dentro
  // de un subconjunto de filas ya enriquecidas (por ejemplo, las de un centro
  // de costos o un conductor específico).
