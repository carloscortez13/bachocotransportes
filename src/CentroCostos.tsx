...
function CentroCostos({ onVolver }: Props) {
  ...
  const cargarDatos = async () => {
    ...
    while (true) {
      let query = supabase
        ...
        .select("centro_costos, id_vehiculo, conductor, proveedor, fecha, litros, recorrido, rendimiento_estandar, rendimiento_real, costo_litro, costo_total, cumplimiento, estado_transaccion");
  ...
  const toggleCeCo = (centro: string) => {
    setFiltroCeCo(prev => prev.includes(centro) ? prev.filter(c => c !== centro) : [...prev, centro]);
  };
  ...
  const resumenPorCentro: FilaCentro[] = (() => {
    ...
    for (const row of datos) {
      const centro = row.centro_costos || "Sin CeCo";
      if (!mapa[centro]) {
        mapa[centro] = {
          centro, cargas: 0, ok: 0, abajo: 0, arriba: 0, pctCumplimiento: 0,
          dineroEnRiesgo: 0, costoTotal: 0, incidencias: 0, filas: [],
          unidades: new Set(), conductores: new Set(), proveedores: new Set()
        };
      }
    ...
    <div style={{ backgroundColor: TEMA.fondo, minHeight: "100vh", color: TEMA.textoPrincipal, fontFamily: "Arial, sans-serif" }}>
            ...
            {ceCoDropdownOpen && (
              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, marginTop: "4px", backgroundColor: "#fff", border: `1px solid ${TEMA.borde}`, borderRadius: "8px", padding: "8px", zIndex: 10, maxHeight: "280px", overflowY: "auto", width: "300px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}>
                ...
                {ceCoFiltradosBusqueda.map(c => (
                  <label key={c} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "6px 4px", cursor: "pointer", fontSize: "13px", color: TEMA.textoPrincipal }}>
      ...
      <div style={{ padding: "24px 32px", display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: "16px" }}>
        ...
        <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "20px", borderTop: `4px solid ${TEMA.naranja}` }}>
          ...
          <h2 style={{ color: TEMA.textoPrincipal, margin: "8px 0 0", fontSize: "26px" }}>{fmtDinero(costoTotalGlobal)}</h2>
        </div>
        <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "20px", borderTop: `4px solid ${TEMA.naranja}` }}>
          <p style={{ color: TEMA.textoSecundario, margin: 0, fontSize: "14px" }}>$ en Riesgo</p>
          <h2 style={{ color: TEMA.rojo, margin: "8px 0 0", fontSize: "26px" }}>{fmtDinero(totalRiesgo)}</h2>
        </div>
        <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "20px", borderTop: `4px solid ${TEMA.naranja}` }}>
          <p style={{ color: TEMA.textoSecundario, margin: 0, fontSize: "14px" }}>Total Registros</p>
          <h2 style={{ color: TEMA.textoPrincipal, margin: "8px 0 0", fontSize: "30px" }}>{totalCargas}</h2>
        </div>
      </div>
        ...
        {cargando ? <p style={{ color: TEMA.textoSecundario }}>Cargando datos...</p> : (
          <div style={{ backgroundColor: TEMA.fondoTarjeta, borderRadius: "12px", padding: "24px", border: `1px solid ${TEMA.borde}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h3 style={{ color: TEMA.textoPrincipal, marginTop: 0 }}>Resumen por CeCo ({resumenPorCentro.length})</h3>
              <button onClick={() => descargarCSV(resumenPorCentro.map(f => ({
                CeCo: f.centro, Cargas: f.cargas, OK: f.ok, Por_Abajo: f.abajo, Por_Arriba: f.arriba,
              ...
              })), "centro_de_costos")} style={{ backgroundColor: TEMA.verde, border: "none", color: "white", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontSize: "13px", fontWeight: "bold" }}>
                📥 Descargar Excel
              </button>
            </div>
            <p style={{ color: TEMA.textoSecundario, fontSize: "13px", marginTop: "4px", marginBottom: "16px" }}>Da clic en un CeCo para ver el detalle de cargas.</p>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "1000px" }}>
                <thead>
                  <tr style={{ borderBottom: `1px solid ${TEMA.borde}` }}>
                    <th style={{ color: TEMA.textoSecundario, textAlign: "left", padding: "12px", fontWeight: "normal" }}>CeCo</th>
                ...
                <tbody>
                  {resumenPorCentro.map((fila, i) => {
                    const expandido = centroExpandido === fila.centro;
                    return (
                      <>
                        <tr key={i} onClick={() => setCentroExpandido(expandido ? null : fila.centro)} style={{ borderBottom: `1px solid ${TEMA.borde}`, cursor: "pointer" }}>
                          <td style={{ padding: "12px", color: TEMA.textoPrincipal }}>{expandido ? "▼" : "▶"} {fila.centro}</td>
                        ...
                        {expandido && (
                          <tr>
                            <td colSpan={9} style={{ padding: "16px 24px", backgroundColor: "#ffffff" }}>
                              ...
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                                ...
                                <button onClick={() => descargarCSV(fila.filas.slice().sort((a, b) => (a.fecha || "").localeCompare(b.fecha || "")).map(c => ({
                                  Fecha: c.fecha, Conductor: c.conductor, Unidad: c.id_vehiculo,
                                ...
                                })), `detalle_ceco_${fila.centro}`)} style={{ backgroundColor: TEMA.verde, border: "none", color: "white", padding: "6px 14px", borderRadius: "8px", cursor: "pointer", fontSize: "12px", fontWeight: "bold" }}>
                                  📥 Descargar detalle
                                </button>
                              </div>
                              <div style={{ overflowX: "auto" }}>
